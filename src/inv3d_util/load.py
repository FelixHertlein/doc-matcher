...
def load_npz(file: Path, allow_pickle: bool = True) -> np.ndarray:
    file = check_file(file, suffix=".npz")
    with np.load(file, allow_pickle=allow_pickle) as archive:
...
def load_npy(file: Path) -> np.ndarray:
    file = check_file(file, suffix=".npy")
    return np.load(file)


def save_npy(file: Path, data: np.ndarray, override: bool = False):
    exist = None if override else False
    file = check_file(file, suffix=".npy", exist=exist)

    temp_file = file.with_name(f"{file.stem}_{os.getpid()}.temp.npy")

    np.save(temp_file, data)
    os.rename(temp_file, file)  # Atomically rename
...
def save_npz(file: Path, data: np.ndarray, override: bool = False):
    ...
    temp_file = file.with_name(f"{file.stem}_{os.getpid()}.temp.npz")

    params = {file.stem: data}
    np.savez_compressed(str(temp_file), **params)
    os.rename(temp_file, file)  # Atomically rename
