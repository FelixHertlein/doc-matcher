...
@lru_cache(maxsize=1)
def load_model_cached(config, ckpt, device):
    return load_model(config, ckpt, device)
...
def do_instance(model, img, score_thr=0.3):
    ...
    return {
        class_name: parse_result(
            bbox=result[0][class_idx],
            masks=result[1][class_idx],
...
def parse_result(bbox, masks, score_thresh=0.3):
...
def get_dataseries(
    ...
    for line_mask in inst_masks:
        ...
        line_ds = line_utils.get_kp(
            line_mask,
            interval=mask_kp_sample_interval,
            x_range=x_range,
            get_num_lines=False,
            get_center=True,
        )
