...
from inv3d_util.mapping import invert_map, apply_map, scale_map, create_identity_map
...
class ForwardMap:
    ...
    def __init__(
        ...
        top_border = LineString(
            top_border.interpolate(i, normalized=True) for i in np.linspace(0, 1, 100)
        )
        bottom_border = LineString(
            bottom_border.interpolate(i, normalized=True)
            for i in np.linspace(0, 1, 100)
        )
        left_border = LineString(
            left_border.interpolate(i, normalized=True) for i in np.linspace(0, 1, 100)
        )
        right_border = LineString(
            right_border.interpolate(i, normalized=True) for i in np.linspace(0, 1, 100)
        )
    ...
    def add_line(
        self,
        warped_line: LineString,
        template_line: LineString,
        type: str,
        allow_invalid: bool = False,
    ):
        assert allow_invalid, "allow valid only is not implemented"
        self.v_map.add_line(warped_line, template_line, type, allow_invalid)
        self.h_map.add_line(warped_line, template_line, type, allow_invalid)
    ...
    def project_points(self, allow_invalid: bool = False):
        self.v_map.project_points(allow_invalid)
        self.h_map.project_points(allow_invalid)
