...
class ForwardMapChannel:
    ...
    def add_points(
        self, points: List[Point], values: List[float], type: str, allow_invalid: bool
    ):
        for point, value in zip(points, values):
            self.add_point(point, value, type, allow_invalid=allow_invalid)

    def add_point(self, point: Point, value: float, type: str, allow_invalid: bool):
        ...
        if not allow_invalid and (self.badness() > old_self.badness()):
            # print("Invalid point found. Reverting point changes.")
    ...
    def badness(self) -> int:
        if self.is_vertical_gradient:
            diff = self.interp[1:] - self.interp[:-1]
        else:
            diff = self.interp[..., 1:] - self.interp[..., :-1]

        neg_badness = np.sum(diff < self.min_slope)
        pos_badness = np.sum(diff > self.max_slope)
    ...
    def project_points(self, allow_invalid: bool = False):
        ...
        self.add_points(new_points, new_values, "projected", allow_invalid)
