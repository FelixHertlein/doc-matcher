...
class BaseSemanticDataset(VisionDataset):
    ...
    def __init__(
        ...
        print(
            "img_folder_name: {img_folder_name}, ann_folder_name: {ann_folder_name}".format(
                img_folder_name=self.img_path, ann_folder_name=self.ann_path
            )
        )
        self.img_names = [
            img_name.split(img_suffix)[0]
    ...
    def __getitem__(self, index):
        img = Image.open(
            os.path.join(self.img_path, self.img_names[index] + self.img_suffix)
        )
        ann = Image.open(
            os.path.join(self.ann_path, self.img_names[index] + self.ann_suffix)
        )
    ...
    def __len__(self):
        if self.limit_samples:
            return min(len(self.img_names), self.limit_samples)
...
class TorchVOCSegmentation(VOCSegmentation):
    def __init__(
        ...
        super(TorchVOCSegmentation, self).__init__(
            root=root,
            year=year,
            image_set=image_set,
            download=download,
